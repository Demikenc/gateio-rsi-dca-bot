...
def make_exchange(dry_run: bool):
    ...
    exchange = ccxt.gateio({
        ...
        "options": {
            "defaultType": "spot",   # use spot only
        },
    })

    # longer HTTP timeout for slower networks
    exchange.timeout = 20000  # 20s

    # ensure we only load SPOT markets (skip futures/options endpoints)
